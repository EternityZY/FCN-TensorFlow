...
def get_variable(weights, name):
    init = tf.constant_initializer(weights, dtype=tf.float32)       # 常数初始化，按照weights初始化init
...
def weight_variable(shape, stddev=0.02, name=None):
    # print(shape)
    initial = tf.truncated_normal(shape, stddev=stddev)     # 随即初始化
    if name is None:
...
def conv2d_transpose_strided(x, W, b, output_shape=None, stride = 2):
    ...
    if output_shape is None:
        # x.shape = [b,h,w,c]
        # W.shape = [h,w, outc, inc]
        output_shape = x.get_shape().as_list()      # output_shape 如果没有出入，则默认将x扩大两倍
        output_shape[1] *= 2                        # x 中h*2
        output_shape[2] *= 2                        # w * 2
        output_shape[3] = W.get_shape().as_list()[2]    # output_shape中的通道个数 = W中输出通道个数outc
    # print output_shape
